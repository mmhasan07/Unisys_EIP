...
import { sqlCreateRowRoute } from '../../utils/APIRoutes';
...
const Modal = ({ row, setIsModalOpen, fetchData, tableName }) => {
    ...
    const handleCreate = async() => {
        try {
            ...
            const resp = await axios.post(sqlCreateRowRoute, {
                tableName,
                row: localRow
            }, {
                headers: {
            ...
            await fetchData();
            toast.success("Row Created Successfully", toastOptions);
        } catch (err) {
                ...
                <div className="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div className="inline-block align-center bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline" style={{ maxHeight: "85vh", overflowY: "auto" }}>
                                ...
                                Object.keys(row).map((key) => {
                                    return (
                                        <>
                                            ...
                                            <input type="text" className="w-full outline-none rounded bg-gray-100 p-2 mt-2 mb-3" name = {key} onChange={handleChange} />
                                        </>
                                    )
                                })
                            }
                        ...
                        <div className="bg-gray-200 px-4 py-3 text-right">
                            ...
                            <button type="button" className="py-2 px-4 bg-blue-500 text-white rounded font-medium hover:bg-blue-700 mr-2 transition duration-500" onClick={handleCreate}><i className="fas fa-plus"></i> Create</button>
                        </div>
                    </div>
                </div>
            </div>
